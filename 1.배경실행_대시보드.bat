...
if exist "%~dp0node.exe" (
    powershell -Command "Start-Process -FilePath '%~dp0node.exe' -ArgumentList 'server.js' -WorkingDirectory '%~dp0.' -WindowStyle Hidden"
) else (
    where node >nul 2>&1
    if %errorlevel% equ 0 (
        powershell -Command "Start-Process -FilePath 'node' -ArgumentList 'server.js' -WorkingDirectory '%~dp0.' -WindowStyle Hidden"
    ) else (
        powershell -Command "Start-Process -FilePath '%~dp0mps_dashboard_app.exe' -WorkingDirectory '%~dp0.' -WindowStyle Hidden"
    )
)
