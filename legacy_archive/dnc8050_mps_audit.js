...
try {
    ...
    let log = "Searching for DNC 8050 production in Sheet 1 (MPS):\n";
    for (let r = 5; r < mpsRaw.length; r++) {
        const row = mpsRaw[r] || [];
        const prod = (row[4] || '').toString().toUpperCase();
        if (prod.includes('DNC8050') || prod.includes('DNC 8050')) {
            const qtys = mpsMonthIdxs.map((idx, i) => `${monthNames[i]}: ${row[idx]||0}`).join(', ');
            log += `Row ${r}: Product=${row[4]}, Code=${row[3]}, Qtys=[${qtys}]\n`;
        }
    }

    fs.writeFileSync('dnc8050_mps_audit.txt', log);
} catch (e) {
    fs.writeFileSync('dnc8050_mps_audit.txt', 'ERROR: ' + e.message);
}
