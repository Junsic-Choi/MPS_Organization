...
// Re-import the norm function from server.js isn't easy, so I'll copy it here for local test
...
mpsRaw.forEach((row, i) => {
    ...
    if (!prod) return;
    const mNorm = norm(prod);
    mpsPool.push({ code: row[3], product: prod, mNorm, usedIdx: [] });
});

let out = "--- Post-Fix Hutec LYNX XG800 Verification ---\n";
let lastSite = "";
...
fs.writeFileSync('debug_verify_fix.txt', out);
