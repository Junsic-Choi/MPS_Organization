# final_map_v2.ps1
$logFile = "c:\Users\i0215099\Desktop\MPS_UPDATE\mapping_log_ps.txt"
$csvFile = "c:\Users\i0215099\Desktop\MPS_UPDATE\Final_Mapped_Data.csv"
...
Write-Log "Starting Final Mapping PS Version (Fix Numeric)"
...
try {
    ...
    for ($r=6; $r -le 2500; $r++) {
        ...
        if ($mModel -eq "" -and $mProd -eq "") { continue }
                    ...
                    for ($q=1; $q -le $mQty; $q++) {
                        ...
                        foreach ($u in $units) {
                            if (-not $u.Used -and $u.mIdx -eq $m) {
                                $s1 = $u.Model.Replace(" ", "").ToUpper()
                                $s2 = $mModel.Replace(" ", "").ToUpper()
    ...
    Write-Log "Done. Mapped lines (inc header): $($results.Count)"
