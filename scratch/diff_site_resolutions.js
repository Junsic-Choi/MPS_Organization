...
prodRaw.forEach((row, idx) => {
    ...
    if (m) {
        ...
        // Single
        if (!singleMeta[mKey]) {
            singleMeta[mKey] = { site: lastSite, group: lastGroup, model: m, rpm: rpm };
        }
        // Multi
        if (!multiMeta[mKey]) {
            multiMeta[mKey] = [];
        }
        const exists = multiMeta[mKey].some(item => item.site === lastSite && item.model === m);
        if (!exists) {
            multiMeta[mKey].push({ site: lastSite, group: lastGroup, model: m, rpm: rpm });
        }
    }
});
...
function resolveSingle(mKey, originalSite) {
    let foundMeta = singleMeta[mKey];
    if (!foundMeta) {
        const possibleKeys = Object.keys(singleMeta);
        const bestMatch = possibleKeys.find(k => {
            if (k.length <= 1 || mKey.length <= 1) return false;
            return k.startsWith(mKey) || mKey.startsWith(k);
        });
        if (bestMatch) foundMeta = singleMeta[bestMatch];
    }
    return foundMeta ? foundMeta.site : originalSite;
}

function resolveMulti(mKey, originalSite) {
    let mpsMainPlant = '남산';
    if (originalSite === '1842' || originalSite.includes('성주')) {
        mpsMainPlant = '성주';
    }

    let foundMetaList = multiMeta[mKey];
    if (!foundMetaList || foundMetaList.length === 0) {
        const possibleKeys = Object.keys(multiMeta);
        const bestMatch = possibleKeys.find(k => {
            if (k.length <= 1 || mKey.length <= 1) return false;
            return k.startsWith(mKey) || mKey.startsWith(k);
        });
        if (bestMatch) foundMetaList = multiMeta[bestMatch];
    }
...
masterRaw.forEach((row, idx) => {
    ...
    const siteSingle = resolveSingle(mKey, originalSite).replace(/^\d+\.\s*/, '').trim();
    const siteMulti = resolveMulti(mKey, originalSite).replace(/^\d+\.\s*/, '').trim();

    if (siteSingle !== siteMulti) {
        console.log(`MPS Row ${idx}: Product="${pName}", Key="${mKey}", Original="${originalSite}", SingleResolved="${siteSingle}", MultiResolved="${siteMulti}"`);
    }
});
