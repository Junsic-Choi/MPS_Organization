...
masterRaw.forEach((row, idx) => {
    ...
    if (originalSite === '1842' && finalSite === '남산') {
        masterMonthCols.forEach(mCol => {
            ...
            if (q > 0) {
                sum1842Mismapped += q;
                console.log(`MPS Row ${idx}: Product=${pName}, Month=${mCol.name}, Qty=${q}`);
            }
        });
    }
});

console.log('Total 1842 Qty mapped to 남산 (excluding 10월):', sum1842Mismapped);
