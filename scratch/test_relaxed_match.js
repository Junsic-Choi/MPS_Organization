...
// Build Meta Map with relaxed condition (allowing empty rpm)
const metaMap = {};
...
prodRaw.forEach((row, idx) => {
    ...
    // RELAXED CONDITION: allow empty RPM, but must have a valid model name
    if (lastMetaModel) {
...
// Let's check what matches for DC3710F, DC378F2, DCM315H
const testKeys = ['DC3710F', 'DC378F2', 'DC428F', 'DC421F2', 'DCM315H', 'DCM316H'];
testKeys.forEach(tk => {
    const mKey = getMatchKey(tk);
    console.log(`\nMatchKey for "${tk}" is "${mKey}"`);
    const matches = metaMap[mKey];
    if (matches) {
        matches.forEach(match => {
            console.log(`  -> Match: Model="${match.model}", Group="${match.group}", Site="${match.site}", RPM="${match.rpm}"`);
        });
    } else {
