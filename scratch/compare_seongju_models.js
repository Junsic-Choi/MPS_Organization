...
// Sum 생산배포용 (Excluding 10월, columns 4, 7, 8, 9, 10, 12)
const prodSeongjuModels = {};
let lastProdSite = '';
let lastProdModel = '';
prodRaw.forEach((row, idx) => {
    if (idx <= pCols.headerRowIdx) return;
    ...
    const m = (row[pCols.Model] || '').toString().trim();
    if (s) lastProdSite = s;
    if (m) lastProdModel = m;

    const cleanSite = lastProdSite.replace(/^\d+\.\s*/, '').trim();
    ...
    if (cleanSite === '성주') {
        ...
        if (rowSum > 0) {
            prodSeongjuModels[lastProdModel] = (prodSeongjuModels[lastProdModel] || 0) + rowSum;
        }
    }
});

// Sum MPS (Excluding 10월)
let monthRowIdx = -1;
let typeRowIdx = -1;
...
masterRaw.forEach((row, idx) => {
    ...
    if (finalSite === '성주' && (pName || mModel)) {
        masterMonthCols.forEach(mCol => {
            ...
            if (q > 0) {
                // Determine model name in final result
                const modelPart = (pName || mModel).split('-')[0].trim();
                let canonicalModel = modelPart;
                if (foundMeta && foundMeta.model) canonicalModel = foundMeta.model;

                mpsSeongjuModels[canonicalModel] = (mpsSeongjuModels[canonicalModel] || 0) + q;
            }
        });
    }
});

console.log('Seongju Models comparison:');
console.log('Model Name | 생산배포용 (4~9월) | MPS (4~9월)');
const allModels = new Set([...Object.keys(prodSeongjuModels), ...Object.keys(mpsSeongjuModels)]);
allModels.forEach(m => {
    console.log(`${m.padEnd(25)} | ${(prodSeongjuModels[m] || 0).toString().padStart(15)} | ${(mpsSeongjuModels[m] || 0).toString().padStart(12)}`);
});
