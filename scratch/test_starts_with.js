...
masterRaw.forEach((row, idx) => {
    ...
    if (!foundMeta) {
        ...
        // Safely check using startsWith instead of includes
        const bestMatch = possibleKeys.find(k => {
            if (k.length <= 1 || mKey.length <= 1) return false; // Prevent matching single characters
...
console.log('Site counts with startsWith prefix fallback:');
