...
console.log('pCols detected:', pCols);
...
console.log('metaMap key count:', Object.keys(metaMap).length);
console.log('Some entries in metaMap:');
let count = 0;
for (const [k, v] of Object.entries(metaMap)) {
    if (count++ < 10) console.log(`  "${k}":`, v);
}

// Let's analyze master raw
...
// Let's extract and count sites
const sites = new Set();
const finalResults = [];

// ... extract month cols logic
let monthRowIdx = -1;
let typeRowIdx = -1;
...
if (monthRowIdx !== -1 && typeRowIdx !== -1) {
    ...
    typeRow.forEach((cell, idx) => {
        ...
        if (type === '생산') {
            for (let c = idx; c >= 0; c--) {
                const mNum = monthRow[c];
                if (mNum) {
                    masterMonthCols.push({ name: mNum, col: idx });
                    break;
                }
            }
        }
...
masterRaw.forEach((row, idx) => {
    ...
    if (pName || mModel) {
        sites.add(finalSite || '기타');
    }
});

console.log('Distinct sites found:', Array.from(sites));
