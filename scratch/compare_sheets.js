...
// Sum 생산배포용 (Excluding 10월)
// Col 4 is 4월 실적
// Col 7, 8, 9, 10, 12 are 5, 6, 7, 8, 9월
const prodSiteSums = {};
let lastProdSite = '';
prodRaw.forEach((row, idx) => {
    if (idx <= pCols.headerRowIdx) return;
    ...
    if (rowSum > 0 && row[pCols.Model]) {
        prodSiteSums[cleanSite] = (prodSiteSums[cleanSite] || 0) + rowSum;
    }
});

// Sum MPS (Excluding 10월)
// In master, monthRowIdx=2, typeRowIdx=4
let monthRowIdx = -1;
let typeRowIdx = -1;
...
masterRaw.forEach((row, idx) => {
    ...
    if (pName || mModel) {
        masterMonthCols.forEach(mCol => {
            ...
            if (q > 0) {
                const sKey = finalSite || '기타';
                mpsSiteSums[sKey] = (mpsSiteSums[sKey] || 0) + q;
            }
        });
    }
});

console.log('Site sums comparison:');
console.log('Site name | 생산배포용 (Col 4+7+8+9+10+12) | MPS (4~9월)');
const allSites = new Set([...Object.keys(prodSiteSums), ...Object.keys(mpsSiteSums)]);
allSites.forEach(s => {
    console.log(`${s.padEnd(10)} | ${(prodSiteSums[s] || 0).toString().padStart(30)} | ${(mpsSiteSums[s] || 0).toString().padStart(10)}`);
});
